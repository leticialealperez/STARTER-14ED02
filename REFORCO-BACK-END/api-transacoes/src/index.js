...
app.post(
  ...
  (request, response) => {
    ...
    return response.status(201).json({
      mensagem: `Transação realizada com sucesso. Seu novo saldo é de R$ ${usuarioEncontrado.carteira.saldo.toFixed(
        2
      )}`,
...
app.get("/transacoes/:idTransacao", validaToken, (request, response) => {
  ...
  const transacaoEncontrda = usuarioEncontrado.carteira.transacoes.find(
    (trans) => trans.id === idTransacao
  );
...
app.get("/transacoes", validaToken, validaFiltros, (request, response) => {
  ...
  if (tipoTransacao) {
    listaTransacoesFiltrada = listaTransacoesFiltrada.filter(
      (transacao) => transacao.tipo === tipoTransacao
    );
  }

  if (valorMin && valorMax) {
    listaTransacoesFiltrada = listaTransacoesFiltrada.filter(
      (transacao) => transacao.valor >= valorMin && transacao.valor <= valorMax
    );
    ...
    return response.status(200).json({
      ...
      dados: {
        ...
        transacoes: listaTransacoesFiltrada.map(
          ({ id, valor, tipo, dataLancamento, descricao }) => ({
            id,
            valor,
            tipo,
            dataLancamento,
  ...
  if (valorMin) {
    listaTransacoesFiltrada = listaTransacoesFiltrada.filter(
      (transacao) => transacao.valor >= valorMin
    );
  }

  if (valorMax) {
    listaTransacoesFiltrada = listaTransacoesFiltrada.filter(
      (transacao) => transacao.valor <= valorMax
    );
  }
...
app.put("/transacoes/:idTransacao", validaToken, (request, response) => {
  ...
  if (novoSaldo < 0) {
    return response.status(400).json({
      mensagem:
        "Não é possível modificar a transação informada por não possui saldo suficiente para o decremento.",
    });
  }

  usuarioEncontrado.carteira.saldo = novoSaldo

  usuarioEncontrado.carteira.transacoes[indiceEncontrado] =
    listaCopia[indiceEncontrado];
...
app.delete(`/transacoes/:id`, validaToken, (request, response) => {
  ...
  if (novoSaldo < 0) {
    return response.status(400).json({
      mensagem:
        "Não é possível excluir a transação pois sua carteira ficará negativa.",
    });
  }
  ...
  return response.status(200).json({
    mensagem: `Transação excluida com sucesso. Seu novo saldo é de ${usuarioEncontrado.carteira.saldo.toLocaleString(
      "pt-BR",
